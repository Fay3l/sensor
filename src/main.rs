...
async fn main() {
    ...
    // Connecter le capteur
    {
        ...
        sensor.lock().await.set_engineering_mode().await;
    }
...
#[axum::debug_handler]
async fn sse_handler(State(sensor): State<Arc<Mutex<ld2410c::Ld2410C>>>) -> Sse<impl Stream<Item = Result<Event, Infallible>>> {
